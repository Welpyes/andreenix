...
# wallpaper ---------------------------------
feh --bg-scale $HOME/.local/share/backgrounds/vietnam.png

# polybar ---------------------------------
sed -i -e 's/STYLE=.*/STYLE="gruvbox"/g' $polybar_path/launch.sh
...
# rofi ---------------------------------
sed -i -e 's/STYLE=.*/STYLE="gruvbox"/g' "$rofi_path/bin/mpd" "$rofi_path/bin/network" "$rofi_path/bin/battery"
sed -i -e 's/DIR=.*/DIR="gruvbox"/g' "$rofi_path/bin/launcher" "$rofi_path/bin/powermenu"
...
# geany ---------------------------------
sed -i -e 's/color_scheme=.*/color_scheme=monokai.conf/g' "$geany_path"/geany.conf
sed -i -e 's/editor_font=.*/editor_font=Iosevka Custom 10/g' "$geany_path"/geany.conf

# gtk theme, icons and fonts ---------------------------------
xfconf-query -c xsettings -p /Net/ThemeName -s "Gruvbox-Dark-Compact-Soft"
...
obconfig () {
	...
	theme="Gruvbox-Dark-Compact-Soft"
	layout="MLC"
	font="Noto Sans"
	fontsize="9"
